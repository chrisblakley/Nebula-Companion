...
if ( !trait_exists('Companion_Prototyping') ){
	trait Companion_Prototyping {
		public function hooks(){
			if ( nebula()->get_option('prototype_mode') ){
				...
				if ( is_plugin_active('jonradio-multiple-themes/jonradio-multiple-themes.php') ){
					...
					$mt_settings['query'] = array(
						'phase' => array(
							'wireframe' => nebula()->get_option('wireframe_theme'), //Wireframe Theme
							'staging' => nebula()->get_option('staging_theme'), //Staging Theme
							'production' => nebula()->get_option('production_theme') //Production Theme
						)
					);

					$mt_settings['current'] = nebula()->get_option('production_theme'); //"Everything Else" theme
					$mt_settings['ajax_all'] = ( nebula()->get_option('staging_theme') )? nebula()->get_option('staging_theme') : nebula()->get_option('production_theme'); //Theme for AJAX functions. Staging (if exists) or Production
			...
			//@todo: This needs to go in the Sass "Skip file" checks (via a hook):
		...
		public function enqueue_nebula_wireframing(){
			if ( $this->dev_phase() === 'wireframe' ){
				wp_register_style('nebula-wireframing', plugin_dir_path(__FILE__) . '/assets/css/wireframing.css', array('nebula-main'), null);
				nebula()->register_script('nebula-wireframing', plugin_dir_path(__FILE__) . '/assets/js/wireframing.js', 'defer', array('nebula-main'), null, true);
		...
		public function admin_bar_nebula_wireframing($wp_admin_bar){
			...
			if ( $this->dev_phase() ){
				...
				if ( $this->dev_phase() !== 'wireframe' && nebula()->get_option('wireframe_theme') ){
					$wp_admin_bar->add_node(array(
				...
				if ( $this->dev_phase() !== 'staging' && nebula()->get_option('staging_theme') ){
					$wp_admin_bar->add_node(array(
						'parent' => 'nebula-prototype',
				...
				if ( ($this->dev_phase() !== 'production' || nebula()->is_admin_page()) ){
					$wp_admin_bar->add_node(array(
						'parent' => 'nebula-prototype',
